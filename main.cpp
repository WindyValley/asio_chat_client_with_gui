...
int main(int argc, char **argv)
{
  ...
  g_signal_connect(G_OBJECT(win->window), "delete_event", G_CALLBACK(close),
                   NULL);
  g_signal_connect(G_OBJECT(login->dialog), "delete_event", G_CALLBACK(exit),
                   NULL);
  g_signal_connect(G_OBJECT(win->snd_button), "clicked", G_CALLBACK(cb_button),
                   win->snd_msg);
  g_signal_connect(G_OBJECT(login->cnbutton), "clicked",
                   G_CALLBACK(cb_cnbutton), NULL);
  g_signal_connect(G_OBJECT(login->ccbutton), "clicked", G_CALLBACK(exit),
                   NULL);
  loop.join();
  return 0;
}
...
void handler(boost::system::error_code ec, std::size_t length)
{
  if (!ec)
  {
    ...
    gtk_text_buffer_insert(win->buffer, &end, client->read_msg_.body(),
